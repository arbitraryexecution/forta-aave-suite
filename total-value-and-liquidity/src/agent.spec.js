...
const baseRollingMath = {
  ...
  getNumElements: jest.fn().mockReturnValue(0),
  getSum: jest.fn().mockReturnValue(new BigNumber(0)),
  getAverage: jest.fn().mockReturnValue(new BigNumber(0)),
  getStandardDeviation: jest.fn().mockReturnValue(new BigNumber(0)),
  addElement: jest.fn().mockReturnValue(new BigNumber(0)),
};
...
describe('liquidity and total value locked agent tests', () => {
  ...
  describe('configurations work for', () => {
    ...
    it('window size', async () => {
      ...
      expect(mockRollingMath).toBeCalledWith(initializeData.windowSize);
    });
    ...
    */
    it('standard deviation limit', async () => {
      ...
      mockRollingMathFuncs.getNumElements.mockReturnValue(initializeData.minElements + 1);

      // make our math module return predictable values
      mockRollingMathFuncs.getAverage.mockReturnValue(new BigNumber(10));
      mockRollingMathFuncs.getStandardDeviation.mockReturnValue(new BigNumber(1));
    ...
    it('minimum elements before triggering', async () => {
      mockRollingMathFuncs.getNumElements.mockReturnValue(0);
      // run a block to initialize our data
      await handleBlock({ blockNumber: 0 });
  ...
  describe('doesn\'t alert when', () => {
    it('observation isn\'t outside standard deviation limit', async () => {
      ...
      mockConfig.numStdDeviations = 10;
